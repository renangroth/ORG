...
def leitura(acertos_L, falhas_L): # funcao para ler um endereco de memoria
	...
	for linha in cache:
		if num_block == linha[1]: # verifica se o bloco obtido pelo endereco fornecido esta em alguma linha da cache
			...
			print("Linha: ")
			print(cache[i])
			cache[i] = [line, num_block, linha[2], linha[3]+1]
			...
			for item2 in cache:
				if menor == 0:
					...
					if item2[3] < menor:
						...
						line2 = item2[0]
		if(item[3] == menor):
			del cache[i] # remove a linha da cache mais antiga
			j = 0
			...
			print("Linha: ")
			print(item)
			cache.append([line2, num_block, num, 1]) # adiciona a tupla nova a cache
			cache_lines[line2] = lista # adiciona o novo bloco a sua respectiva linha
	...
	if R == 'Not': # O dado que esta sendo inserido não esta na cache
			...
			for item2 in cache:
				if menor == 0:
					...
					if item2[3] < menor:
						...
						line2 = item2[0]
			if(item[3] == menor):
				del cache[i] # remove a linha da cache 
		...
		for item3 in cache:
			if(item3[0] == R):
				...
				print("Linha: ")
				print(cache[i])
				cache[i] = [item[0], num_block, num, cont+1]
				#transforma o endereco em uma string em binario sem o '0b'
				#num = replace_end(num, size)
				#falta alterar os bits de dirty e valid
